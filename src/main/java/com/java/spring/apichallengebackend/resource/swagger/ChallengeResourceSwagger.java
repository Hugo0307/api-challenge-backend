...
import com.java.spring.apichallengebackend.resource.UpdateDescriptionRequest;
...
public interface ChallengeResourceSwagger {
    ...
    @ApiResponses(value = {
                    @ApiResponse(responseCode = "204", description = "Operação realizada com sucesso", content = @Content(schema = @Schema(implementation = MovieDto.class))),
    ...
    @ApiResponses(value = {
                    @ApiResponse(responseCode = "200", description = "Operação realizada com sucesso", content = @Content(schema = @Schema(implementation = MovieDto.class))),
                    @ApiResponse(responseCode = "500", description = "Ocorreu um erro interno durante operação", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
    @GetMapping("/movies")
    ResponseEntity<List<MovieDto>> listMovies();
    ...
    ResponseEntity<MovieDto> getMovie(@Parameter(description = "Id do episódio a ser consultado", required = true) @PathVariable Long episodeId);
}
