...
public interface SagaStarWarsRepository extends JpaRepository<StarWarsEntity, Long> {
    ...
    @Modifying
    @Query("update movie m set m.opening_crawl = :opening_crawl, m.version = :version where m.episode_id = :episode_id")
    void updateDescriptionMovie(@Param(value = "episode_id") Long episode_id, 
                                @Param(value = "opening_crawl") String opening_crawl,
                                @Param(value = "version") String version);

    @Query("select version from movie where episode_id = :episode_id")
    Optional<String> getVersion(@Param(value = "episode_id") Long episode_id);
}
